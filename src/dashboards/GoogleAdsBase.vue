...
const props = defineProps({
  ...
  // URLs das fontes — definidas pelo wrapper de cada cliente
  fonteAdsUrl:      { type: String, required: true },
  fonteKeywordsUrl: { type: String, required: true },
})

// ── URLs das fontes — vêm das props definidas pelo wrapper de cada cliente ────
const isLoading         = computed(() => props.fontsLoading[props.fonteAdsUrl] ?? false)
const isKeywordsLoading = computed(() => props.fontsLoading[props.fonteKeywordsUrl] ?? false)

const rawData = computed(() => {
  const d = props.fontsData[props.fonteAdsUrl]
  return Array.isArray(d) ? d : []
})
const rawKeywords = computed(() => {
  const d = props.fontsData[props.fonteKeywordsUrl]
  return Array.isArray(d) ? d : []
})
