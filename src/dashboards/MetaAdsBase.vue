          ...
          <tbody>
            <tr v-for="(row, i) in campaignRows" :key="i">
              ...
              <td class="text-right">
                ...
                <div v-if="row.resultType" class="result-type-label">
                  <template v-if="formatResultType(row.resultType)">{{ formatResultType(row.resultType) }}</template>
                  <span v-else class="result-type-unknown" :title="row.resultType">
                    <i class="bi bi-exclamation-triangle-fill"></i> {{ row.resultType }}
                  </span>
                </div>
              </td>
...
const props = defineProps({
  ...
  // URL da fonte — definida pelo wrapper de cada cliente
  fonteUrl:     { type: String, required: true },
})

// ── URLs das fontes / serviços ───────────────────────────────────────────────
// FONTE_URL vem da prop `fonteUrl` — definida pelo wrapper de cada cliente
const IMAGE_RESOLVER_URL = 'https://nexus-n8n.2wdiso.easypanel.host/a/imageFace'

const isLoading = computed(() => props.fontsLoading[props.fonteUrl] ?? false)

const rawData = computed(() => {
  const d = props.fontsData[props.fonteUrl]
...
// ── Lógica de resultado dinâmico por objetivo da campanha ────────────────────
// Usa os novos campos: Campaign Result Value + Campaign Result Type
const getResultado  = (r) => Number(r['Campaign Result value']) || 0
const getResultType = (r) => r['Campaign Result Type'] || ''

// ── Mapa de tipos de resultado — atualizar aqui quando surgirem novos tipos ────────
const RESULT_TYPE_LABELS = {
  ...
  'offsite_conversion.fb_pixel_initiate_checkout':           'Checkout Iniciado',
  'offsite_conversion.fb_pixel_custom.EndForm':              'Form Completo',
  'onsite_conversion.messaging_conversation_started_7d':     'Msg Iniciada',
  'video_thruplay_watched_actions':                          'ThruPlay',
  'profile_visit_view':                                      'Visita ao Perfil',
  'reach':                                                   'Alcançe',
  'link_click':                                              'Clique no Link',
}

// Retorna o label amigável ou null se não mapeado (null aciona sinalização no front)
const formatResultType = (type) => {
  if (!type) return null
  return RESULT_TYPE_LABELS[type] ?? null
}

// ── KPIs globais (6 cards) ────────────────────────────────────────────────────
...
const campaignRows = computed(() => {
  ...
  for (const r of rawData.value) {
    ...
    if (!map[key]) map[key] = { campaign: key, cost: 0, resultado: 0, linkClicks: 0, impressions: 0, resultType: '' }
    ...
    // Captura o tipo de resultado da campanha (usa o primeiro encontrado)
    if (!map[key].resultType && getResultType(r)) map[key].resultType = getResultType(r)
  }
...
const adSetRows = computed(() => {
  ...
  for (const r of rawData.value) {
    ...
    if (!map[key]) map[key] = {
      ...
      cost: 0, resultado: 0, linkClicks: 0, impressions: 0, resultType: '',
    }
    map[key].cost        += Number(r['Amount Spent (BRL)']) || 0
    map[key].resultado   += getResultado(r)
    map[key].linkClicks  += Number(r['Link Clicks'])        || 0
...
/* ── Tipo de resultado dinâmico por campanha ── */
.result-type-label {
  font-size: 10px;
  font-weight: 700;
  text-transform: uppercase;
...
/* Label não mapeado — sinaliza em amarelo para facilitar identificação */
.result-type-unknown {
  display: inline-flex;
  align-items: center;
  gap: 3px;
  ...
  border: 1px dashed rgba(223, 166, 37, 0.4);
