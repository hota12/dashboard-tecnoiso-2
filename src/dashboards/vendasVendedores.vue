      ...
      <div class="bottom-analytics-grid">
        ...
        <div class="andamento-detalhe-card">
          ...
          <div class="andamento-detalhe-grid">
            ...
            <!-- Negociação -->
            <div class="andamento-detalhe-item" style="border-color:rgba(171,71,188,0.35);">
              <div class="andamento-detalhe-icon" style="background:rgba(171,71,188,0.12);color:#ab47bc;">
                <i class="bi bi-chat-dots"></i>
              </div>
              <div class="andamento-detalhe-body">
                <span class="andamento-detalhe-label">Em Negociação</span>
                <span class="andamento-detalhe-value">{{ andamentoDetalhe.negociacao.count }}</span>
                <span class="andamento-detalhe-sub">{{ formatCurrency(andamentoDetalhe.negociacao.valor) }}</span>
              </div>
            </div>
            <!-- Outros -->
        ...
        <div class="canais-card">
          <div class="section-header">
            ...
            <div class="micro-toggle">
              <button :class="{ active: filterCidade === 'novos' }" @click="filterCidade = 'novos'">Novos</button>
              <button :class="{ active: filterCidade === 'ganhos' }" @click="filterCidade = 'ganhos'">Ganhos</button>
            </div>
          </div>
          <div v-if="!negociosCidade.length" class="funil-empty">Sem dados de cidade.</div>
          <div v-else class="funil-list">
            <div v-for="item in negociosCidade" :key="item.label" class="funil-item">
              <div class="funil-item-header">
                <span class="funil-label">{{ item.label }}</span>
                <span class="funil-count" style="color:var(--color-text);">{{ item.count }}</span>
              </div>
              <div class="funil-track">
                <div class="funil-fill" style="background:#26a69a" :style="{ width: item.pct + '%' }"></div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="canais-card">
          <div class="section-header">
            ...
            <span class="badge-pill badge-green">{{ kpis.ganhos }} ganho(s)</span>
          </div>
          <div v-if="!negociosPorCategoria.length" class="funil-empty">
            Sem categorias registradas nos negócios ganhos{{ selectedVendedor ? ' para este vendedor' : '' }}.
          </div>
          <div v-else class="funil-list">
...
const FONT_KEY = 'https://domador-n8n.2wdiso.easypanel.host/webhook/1705acd3-9821-4f2d-b524-c9024ab35cc6'
...
const kpis = computed(() => {
  ...
  const andamentoAtrasados = negociosAndamento.filter(l => {
    const isTargetFase = l.faseAtual === 'NEGOCIAÇÃO' || l.faseAtual === 'ORÇAMENTO'
...
const andamentoDetalhe = computed(() => {
  ...
  filtered.value.negociosAndamento.forEach(l => {
    ...
    if (l.faseAtual === 'ORÇAMENTO') {
      result.orcamento.count++
      result.orcamento.valor += val
    } else if (l.faseAtual === 'NEGOCIAÇÃO') {
      result.negociacao.count++
      result.negociacao.valor += val
    } else {
      result.outros.count++
...
const chartValor = computed(() => {
  const { labels, values } = buildAccSeries(filtered.value.negociosGanhos, 'dataVenda', 'valorPedido')
...
// ─── Seção 3: Funil por Fase ───────────────────────────────────
const FASE_ORDER  = ['INÍCIO', 'ORÇAMENTO', 'NEGOCIAÇÃO', 'FECHADO', 'PERDIDO']
const FASE_COLORS = {
  'INÍCIO':     '#42a5f5',
  'ORÇAMENTO':  '#ffa726',
  'NEGOCIAÇÃO': '#ab47bc',
...
// Negócios por Cidade
function normalizeCidade(l) {
  return l.cidade || l.empresa || 'Não informado'
}
const leadsParaCidade = computed(() =>
  filterCidade.value === 'novos' ? filtered.value.novosNegocios : filtered.value.negociosGanhos
)
const negociosCidade = computed(() => {
  const map = {}
  leadsParaCidade.value.forEach(l => {
    const cidade = normalizeCidade(l)
...
function normalizeClienteDaCasa(val) {
  ...
  if (val === false || val === 'false' || val === 'Não') return 'Não'
...
// Ganhos por Categoria
function parseCategoria(catStr) {
...
const negociosPorCategoria = computed(() => {
  const map = {}
  filtered.value.negociosGanhos.forEach(l => {
    parseCategoria(l.categoria).forEach(cat => {
      map[cat] = (map[cat] ?? 0) + 1
    })
  })
  const entries = Object.entries(map).sort((a, b) => b[1] - a[1])
