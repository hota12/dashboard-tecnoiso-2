<template>
  <div class="prevendas-dashboard">
    ...
    <template v-else-if="raw">
      ...
      <!-- Taxa de conversão (linha separada, menor) -->
      <div class="taxa-row">
      ...
      <!-- ── Novas Análises (Vendedor, Unidade, Tipo, Cliente da Casa) ── -->
      <div class="dist-row">
        ...
        <div class="origem-card">
          <div class="origem-header">
            <span class="origem-title">
              ...
              Leads ganhos por vendedor (seleção manual)
            </span>
          </div>
          <div class="origem-list">
        ...
        <div class="chart-card">
          <div class="chart-header">
            <span class="chart-title"><i class="bi bi-shop"></i> Unidade</span>
            <div class="micro-toggle">
              <button :class="{ active: filterUnidade === 'novos' }" @click="filterUnidade = 'novos'">Novos</button>
              <button :class="{ active: filterUnidade === 'ganhos' }" @click="filterUnidade = 'ganhos'">Ganhos</button>
            </div>
          </div>
          <div class="chart-wrapper" style="height: 180px; display: flex; justify-content: center; align-items: center;">
            <Doughnut v-if="leadsUnidade.length" :data="chartUnidade" :options="doughnutOptions" />
            <div v-else class="origem-empty" style="width:100%; text-align:center;">Sem dados</div>
          </div>
        </div>
        ...
        <div class="chart-card">
          <div class="chart-header">
            <span class="chart-title"><i class="bi bi-person-badge"></i> Tipo de Cliente</span>
            <div class="micro-toggle">
              <button :class="{ active: filterTipo === 'novos' }" @click="filterTipo = 'novos'">Novos</button>
              <button :class="{ active: filterTipo === 'ganhos' }" @click="filterTipo = 'ganhos'">Ganhos</button>
            </div>
          </div>
          <div class="chart-wrapper" style="height: 180px; display: flex; justify-content: center; align-items: center;">
            <Doughnut v-if="leadsTipo.length" :data="chartTipo" :options="doughnutOptions" />
            <div v-else class="origem-empty" style="width:100%; text-align:center;">Sem dados</div>
          </div>
        </div>
      ...
      <div v-if="modal.open" class="modal-overlay" @click.self="modal.open = false">
        <div class="modal modal-leads">
          ...
          <div class="modal-filters">
            ...
            <select v-model="modal.filterUnidade" class="form-select filter-select">
              <option value="">Todas as unidades</option>
              <option v-for="u in modalUnidades" :key="u" :value="u">{{ u }}</option>
            </select>
            <select v-model="modal.filterTipo" class="form-select filter-select">
              <option value="">Todos os tipos</option>
              <option v-for="t in modalTipos" :key="t" :value="t">{{ t }}</option>
            </select>
                ...
                <tr v-for="lead in modalFiltered" :key="lead.identificador">
                  <td v-for="col in modal.columns" :key="col.key">
                    ...
                    <template v-else-if="col.key === 'clienteDaCasa'">
                      <span v-if="lead.clienteDaCasa" :class="lead.clienteDaCasa === 'Sim' ? 'badge badge-success' : 'badge badge-dark'">
                        {{ lead.clienteDaCasa }}
                      </span>
...
// ─── Key da fonte (URL completa = chave do fontsData) ──────────
const FONT_KEY = 'https://domador-n8n.2wdiso.easypanel.host/webhook/19a5da67-0255-403a-b148-73a971670ade'
...
// ─── Gera todos os dias de um intervalo ───────────────────────
...
// ─── Acumula leads por dia ─────────────────────────────────────
function buildAccumulatedSeries(leads, dateField = 'criadoEm') {
  // Monta o eixo X com TODOS os dias do filtro (ou, se não houver filtro, só os dias com dados)
  const { startDate, endDate } = props.filters

  // Agrupa leads por label de dia
  const byDay = {}
  leads.forEach(l => {
  ...
  if (startDate && endDate) {
    // Período filtrado: mostra todos os dias mesmo sem leads
    labels = allDaysInRange(startDate, endDate)
  } else if (leads.length) {
    // Sem filtro: usa apenas os dias com dados, ordenados
    const sorted = leads
...
// Perdidos: agrupa por motivo de desqualificação
...
// ─── Novas Métricas (Vendedor, Unidade, Tipo, Cliente da Casa) ─
const filterClienteCasa = ref('novos')
const filterUnidade = ref('novos')
const filterTipo = ref('novos')

const leadsClienteCasa = computed(() => filterClienteCasa.value === 'novos' ? data.value.novosLeads : data.value.leadsGanhos)
const leadsUnidade = computed(() => filterUnidade.value === 'novos' ? data.value.novosLeads : data.value.leadsGanhos)
const leadsTipo = computed(() => filterTipo.value === 'novos' ? data.value.novosLeads : data.value.leadsGanhos)
...
const distClienteCasa = computed(() => buildFrequency(leadsClienteCasa.value, 'clienteDaCasa', 'Não informado'))
...
const doughnutOptions = {
  ...
  cutout: '65%',
  plugins: {
    legend: { position: 'right', labels: { color: '#9e9e9e', usePointStyle: true, boxWidth: 8, font: { size: 11 } } },
    tooltip: { backgroundColor: '#1a1a1a', titleColor: '#fff', bodyColor: '#ccc' }
  }
}

function buildDoughnutData(leads, field, defaultVal = 'Não informado', colors = ['#42a5f5', '#66bb6a', '#ffa726', '#ef5350', '#ab47bc', '#8d6e63']) {
  const map = {}
  leads.forEach(l => {
    let val = l[field]
    if (!val || String(val).trim() === '') val = defaultVal
...
const chartUnidade = computed(() => buildDoughnutData(leadsUnidade.value, 'unidade', 'Sem Unidade', ['#ff7043', '#5c6bc0', '#26a69a']))
const chartTipo = computed(() => buildDoughnutData(leadsTipo.value, 'tipoCliente', 'Não informado', ['#26a69a', '#ab47bc', '#ffa726']))
...
const modal = reactive({
  ...
  columns: [], leads: [],
  search: '', filterUnidade: '', filterTipo: '',
  sortKey: '', sortDir: 'asc',
})
...
function openModal(type) {
  ...
  modal.search = ''; modal.filterUnidade = ''; modal.filterTipo = ''
  modal.sortKey = ''; modal.sortDir = 'asc'
  modal.open = true
}
...
const modalUnidades = computed(() => {
  const set = new Set(modal.leads.map(l => l.unidade).filter(Boolean))
  return [...set].sort()
})

const modalTipos = computed(() => {
  const set = new Set(modal.leads.map(l => l.tipoCliente).filter(Boolean))
  return [...set].sort()
})
...
const modalFiltered = computed(() => {
  ...
  if (modal.filterUnidade) list = list.filter(l => l.unidade === modal.filterUnidade)
  if (modal.filterTipo)    list = list.filter(l => l.tipoCliente === modal.filterTipo)
...
function exportToExcel() {
  ...
  const dataToExport = modalFiltered.value.map(lead => {
    ...
    modal.columns.forEach(col => {
      ...
      if (col.key === 'utmSource') {
        val = formatSource(val)
      } else if (col.isDate) {
        val = formatDate(val)
      }
  ...
  XLSX.writeFile(wb, `${modal.title.replace(/\s+/g, '_')}_${new Date().getTime()}.xlsx`)
}
</script>
...
.chart-card {
  ...
  flex-direction: column;
  gap: 12px;
  min-width: 0; /* Permite encolher em grids flex/grid */
  width: 100%;
}
...
@media (max-width: 1024px) { .dist-row { grid-template-columns: repeat(2, 1fr); } }
@media (max-width: 600px) { .dist-row { grid-template-columns: 1fr; } }
...
/* ── Badges globais (usados inline pelo sistema global) ───── */
...
.badge-warning { background: rgba(255,167,38,0.15);  color: #e65100; }

/* ── Botão detalhar no KPI ────────────────────────────────── */
